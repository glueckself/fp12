...
addRev' round maxRounds value | round > maxRounds = Failure
                              | otherwise = if (isPal value) then Success(round,value,show value)
...
simplCon (Rail c1 c2 _) = (c1,c2,0)
simplCon (Road c1 c2 _) = (c1,c2,0)

simplCons :: Connections -> SimplConnections
simplCons db = map simplCon db
...
isRoute' db crtList dst = if   elem dst crtList then True
                          else let newList = expandList db crtList in
                               let lenN = length newList in
                               let lenC = length crtList in
                                   if   lenN > lenC then isRoute' db newList dst
                                   else False

isRoute :: Connections -> Country -> Country -> Bool
isRoute db start dest = isRoute' (simplCons db) [start] dest

-- isFeelGoodRoute' :: SimplConnections -> [Country] -> Country -> TravelTime -> Bool
-- isFeelGoodRoute' db crlList dst time = 

isFeelGoodRoute :: Connections -> Country -> Country -> TravelTime -> Bool
isFeelGoodRoute db start dest time = isFeelGoodRoute' (simplCons db) [start] dest time
